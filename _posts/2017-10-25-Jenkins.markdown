...
- 4.构建配置
  ...
  >这里需要手动创建四个对应的plist文件，在打包时需要根据这四个（根据需要创建,如果只需要打测试和发布两种包，只需创建两种即可）exportOption.Plist文件打出对应类型的包（图上已经给出了plist文件的样式,需要根据你打包的类型进行修改，也可以用xcode打包导出后，它会生成ExportOptions.plist文件，copy出来用也行，但注意文件名要跟自动打包脚本中的保持一致）
