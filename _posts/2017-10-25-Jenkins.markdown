...
- 2.管理证书和描述文件插件配置<br/>（使用Keychains and Provisioning Profiles Management插件）
   ...
   ![]({{site.url}}/img/jenkins/Snip20171026_32.png)
	...
	> 这里需要说明，如有多个项目并且证书不同的话，可以在Keychains下面的Add Code Signing Identify添加签名标示；另外把新的的描述文件到桌面，点击最上面的“选择文件”上传，完成后，下面的Provisioning Profiles会出现新增的描述文件名称和对应的UUID.
...
- 4.构建配置
  ...
  >这里需要手动创建四个对应的plist文件，在打包时需要根据这四个（根据需要创建,如果只需要打测试和发布两种包，只需创建两种即可）exportOptions.Plist文件打出对应类型的包（图上已经给出了plist文件的样式,需要根据你打包的类型进行修改，也可以用xcode打包导出后，让xcode生成ExportOptions.plist文件，我们copy出来用也行，但注意文件名要跟自动打包脚本中的保持一致）
